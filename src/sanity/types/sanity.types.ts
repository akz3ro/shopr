...
// Source: ./src/sanity/lib/orders/getOrders.ts
// Variable: orderQuery
// Query: *[    _type == "order" && clerkUserId == $userId] | order(orderDate desc) {      ...,      products[] {        ...,        product->      }    }
export type OrderQueryResult = Array<{
  _id: string;
  _type: "order";
  _createdAt: string;
  _updatedAt: string;
...
// Source: ./src/sanity/lib/products/getProductsByCategoryId.tsx
// Variable: productsByCategoryIdQuery
// Query: *[      _type == "product"      && references($categoryId)      ] | order(title asc)
export type ProductsByCategoryIdQueryResult = Array<{
  _id: string;
  _type: "product";
  _createdAt: string;
  _updatedAt: string;
...
// Source: ./src/sanity/lib/products/getSearchProducts.ts
// Variable: searchProductsQuery
// Query: *[      _type == "product" &&      (        title match $term ||        categories[]->title match $term ||        slug.current match $term      )    ] | order(title asc)
...
// Variable: saleByCouponQuery
// Query: *[_type == "sale" && couponCode == $coupon && isActive == true] | order(startDate desc)[0]
export type SaleByCouponQueryResult = {
  _id: string;
  _type: "sale";
  _createdAt: string;
  _updatedAt: string;
...
declare module "@sanity/client" {
  interface SanityQueries {
    ...
    "*[\n      _type == \"product\"\n      && references($categoryId)\n      ] | order(title asc)": ProductsByCategoryIdQueryResult;
    "*[\n      _type == \"product\" &&\n      (\n        title match $term ||\n        categories[]->title match $term ||\n        slug.current match $term\n      )\n    ] | order(title asc)": SearchProductsQueryResult;
    "*[_type == \"sale\" && couponCode == $coupon && isActive == true] | order(startDate desc)[0]": SaleByCouponQueryResult;
  }
}
